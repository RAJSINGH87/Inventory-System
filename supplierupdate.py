...
import pymysql 
def showsupplierupdate():

    t=tkinter.Tk()
    t.geometry('700x700')
    t.title('Supplier')
    r=Label(t,text='Supplier Update Form',font=('arial',20),fg='green',bg='white')
    r.place(x=120,y=10)
    t.config(bg='brown')
    ...
    def updatedata():
        ...
        xe=e5.get()
        xf=e6.get()
        sql="update supplier set sname='%s',address='%s',email='%s',phone='%s',catid='%s' where supplierid=%d"%(xb,xc,xd,xe,xf,xa)
        cur.execute(sql)
        db.commit()
        db.close()
